...
5. Cancelled
*/
class DeleteMenuItemScreen extends StatefulWidget {
  const DeleteMenuItemScreen({super.key});

  @override
  State<DeleteMenuItemScreen> createState() => _DeleteMenuItemScreenState();
}

class _DeleteMenuItemScreenState extends State<DeleteMenuItemScreen> {
  @override
              ...
              Visibility(
                ...
                */
                child: widget.categoryItemList.isEmpty
                    ...
                    /*
                If list is not empty it a ListView will be returned in which each item show an individual items
                of that category
                */
                    : ListView.builder(
                        shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) {
                          return InkWell(
                            ...
                            child: MenuItemView(
                              ...
                              color: Colors.blue,
                              buttonText: "Delete",
                              //basically this will be called when user press delete button
                              onUpdatePressed: () async {
                                if (isDeleteInProgress) {
                                  showSnackBar(
                                      context: context,
                                      content:
                                          "Already In Progress Please Wait | Check Inernet Connection");
                                  return;
                                }
                                ...
                                showSnackBar(
                                  context: context,
                                  content:
                                      'Item is deleting from menu please wait',
                                  duration: const Duration(seconds: 2),
                                );
                                //call the delete function
                                await menuProvider.deleteFoodItemInCategory(
                                  categoryName: widget.category.id,
                                  foodItemId: widget.category.items[index].id,
                                  context: context,
                                );
